...
PID_FILE="${LOG_DIR}/ravdess_actor01_invalids.pid"
LOG_FILE="${LOG_DIR}/ravdess_actor01_invalids.log"
...
start() {
  ...
  cd "${ROOT_DIR}"
  echo "Starting RAVDESS actor01 invalid-input conversion with chunked V2 path..."
  echo "Log: ${LOG_FILE}"

  nohup "${PYTHON}" -u SCRIPT_TEST/batch_convert_v2.py \
    --sources SCRIPT_TEST/inputs_invalids \
    --targets SCRIPT_TEST/references_ravdess \
    --outputs SCRIPT_TEST/outputs_ravdess_v2 \
    --manifest SCRIPT_TEST/manifests/outputs_ravdess_v2_actor01_invalids.csv \
    --target-actors 1 \
